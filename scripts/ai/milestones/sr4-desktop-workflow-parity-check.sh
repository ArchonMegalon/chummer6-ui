...
verification_exit=0
materializer_exit=0
CHUMMER_WORKFLOW_FAMILY_RELEASE_CHANNEL_PATH="$release_channel_path" bash "$repo_root/scripts/ai/milestones/materialize-sr-workflow-family-execution-receipts.sh" sr4 >/dev/null || execution_exit=$?
CHUMMER_WORKFLOW_FAMILY_RELEASE_CHANNEL_PATH="$release_channel_path" bash "$repo_root/scripts/ai/milestones/materialize-sr-workflow-family-verification-receipts.sh" sr4 >/dev/null || verification_exit=$?
CHUMMER_WORKFLOW_FAMILY_RELEASE_CHANNEL_PATH="$release_channel_path" bash "$repo_root/scripts/ai/milestones/materialize-sr-workflow-family-receipts.sh" sr4 >/dev/null || materializer_exit=$?

python3 - <<'PY' "$repo_root" "$receipt_path" "$oracle_path" "$ledger_path" "$dual_head_tests_path" "$compliance_tests_path" "$ui_gate_tests_path" "$workflow_gate_tests_path" "$workflow_gate_exit" "$execution_exit" "$verification_exit" "$materializer_exit" "$release_channel_path" "$release_channel_source"
...
oracle, oracle_exists = load_regular_json(
    oracle_path, "SR4 oracle", source_artifact_reasons
)
ledger, ledger_exists = load_regular_json(
    ledger_path, "SR4 workflow parity ledger", source_artifact_reasons
)
    ...
    try:
        workflow_release_identity = {
            ...
            **workflow_file_binding(
                release_channel_path, "SR4 workflow release channel receipt"
            ),
        }
    except ValueError as exc:
...
def exact_stage_receipt_map(stage: str) -> dict[str, Path]:
    if set(families) != set(required_family_ids):
        raise ValueError("SR4 workflow ledger family inventory is not exact")
    ...
    for family_id in required_family_ids:
        ...
        expected_reference = {
            ...
            "parity": (
                ...
                f"SR4_WORKFLOW_FAMILY_{family_id}.generated.json"
            ),
        }[stage]
        references = family.get(receipt_key)
        if references != [expected_reference]:
            raise ValueError(
                f"SR4 workflow ledger {receipt_key} target is not canonical for {family_id}"
            )
...
def validate_committed_epoch_chain() -> dict[str, dict[str, object]]:
    ...
    if not workflow_release_identity:
        raise ValueError("SR4 workflow release identity is incomplete")
    execution_result = validate_workflow_stage_manifest(
        manifest_path=workflow_stage_manifest_path(repo_root, "sr4", "execution"),
        repo_root=repo_root,
        edition="sr4",
        stage="execution",
    ...
    verification_result = validate_workflow_stage_manifest(
        manifest_path=workflow_stage_manifest_path(repo_root, "sr4", "verification"),
        repo_root=repo_root,
        edition="sr4",
        stage="verification",
    ...
    parity_result = validate_workflow_stage_manifest(
        manifest_path=workflow_stage_manifest_path(repo_root, "sr4", "parity"),
        repo_root=repo_root,
        edition="sr4",
        stage="parity",
    ...
    for stage in ("verification", "parity"):
        ...
        if any(
            ...
            raise ValueError(
                f"SR4 {stage} manifest identity does not match the execution epoch"
            )
    return results
...
try:
    ...
    append_reason(
        "SR4 committed workflow-family epoch chain is invalid: " + str(exc),
...
if materializer_exit != 0:
    append_reason(
        f"SR4 family receipt materialization exited non-zero: {materializer_exit}",
        materialization_reasons,
    )
if verification_exit != 0:
    append_reason(
        f"SR4 verification receipt materialization exited non-zero: {verification_exit}",
        materialization_reasons,
    )
if execution_exit != 0:
    append_reason(
        f"SR4 execution receipt materialization exited non-zero: {execution_exit}",
        materialization_reasons,
    )
...
try:
    ...
    append_reason(
        "SR4 committed workflow-family epoch chain changed before desktop receipt publication: "
        + str(exc),
        parity_receipt_reasons,
        materialization_reasons,
    )
    payload["status"] = "fail"
    payload["summary"] = (
        "SR4 desktop workflow parity is not yet exhaustively proven against the local Chummer4 oracle."
    )
    payload["evidence"]["workflowFamilyEpochCommitted"] = False
