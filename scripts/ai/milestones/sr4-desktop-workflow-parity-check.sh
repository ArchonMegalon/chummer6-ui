...
if candidate_proof_plane_requested "${sr4_external_plane[@]}"; then
  candidate_proof_require_complete_plane "SR4 workflow parity" "${sr4_external_plane[@]}"
  candidate_proof_external_mode=1
  receipt_path="$CHUMMER_SR4_WORKFLOW_PARITY_OUTPUT_PATH"
  proof_input_root="$CHUMMER_SR4_WORKFLOW_PARITY_PROOF_INPUT_ROOT"
  release_channel_path="$CHUMMER_SR4_WORKFLOW_PARITY_RELEASE_CHANNEL_PATH"
  skip_dependency_materialize=1
  candidate_proof_preflight \
    sr4 "$receipt_path" "$repo_root" "$release_channel_path" "$proof_input_root"
...
python3 - <<'PY' "$repo_root" "$receipt_path" "$oracle_path" "$ledger_path" "$dual_head_tests_path" "$compliance_tests_path" "$ui_gate_tests_path" "$workflow_gate_tests_path" "$workflow_gate_exit" "$execution_exit" "$verification_exit" "$materializer_exit" "$release_channel_path" "$proof_input_root" "$candidate_proof_external_mode"
