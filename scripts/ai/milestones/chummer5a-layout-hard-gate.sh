...
if 'x:Name="RightShellRegion"' not in main_window_text or 'IsVisible="False"' not in main_window_text:
...
if section_payload_index < 0 or 'IsExpanded="False"' not in section_host_text[section_payload_index:section_payload_index + 160]:
    reasons.append('Section host is missing the collapsed "Section Payload" expander contract.')
