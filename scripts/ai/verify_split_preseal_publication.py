...
OWNER_CONTEXT_RECIPE_PATHS = frozenset(
    {
...
def validate_oracle_at_recipe(
    ...
    if (
        ...
        or not isinstance(value.get("consumer"), dict)
    ):
    ...
    source_files = value["consumer"].get("sourceFiles")
    if (
        not isinstance(source_files, dict)
        or not source_files
        or producer["path"] in source_files
    ):
