#!/usr/bin/env python3
from __future__ import annotations

import json
import os
from datetime import datetime, timezone
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[1]
OUTPUT_PATH = Path(
    os.environ.get(
        "CHUMMER_BLAZOR_WORKBENCH_HISTORY_UNDO_STAGED_PROOF_PATH",
        REPO_ROOT / ".codex-studio" / "published" / "BLAZOR_WORKBENCH_HISTORY_UNDO_STAGED_PROOF.generated.json",
    )
)

CHECKS = [
    {
        "id": "product_workbench_history_undo",
        "path": "Chummer.Blazor/Components/Pages/Preview.razor",
        "tokens": [
            "Browser-client history and undo lane",
            "data-workbench-history-undo=\"strip\"",
            "Make rollback and recovery visible.",
            "data-workbench-history-undo-action=\"undo\"",
            "data-workbench-history-undo-action=\"redo\"",
            "data-workbench-history-undo-action=\"snapshot\"",
            "data-workbench-history-undo-action=\"compare\"",
            "data-workbench-history-undo-action=\"restore\"",
            "private const string UndoChangeCommand = \"undo_change\"",
            "private const string RedoChangeCommand = \"redo_change\"",
            "private const string CreateSnapshotCommand = \"create_snapshot\"",
            "private const string CompareChangesCommand = \"compare_changes\"",
            "private const string RestoreSnapshotCommand = \"restore_snapshot\"",
            "private const string ReviewConflictCommand = \"review_conflict\"",
            "command: UndoChangeCommand",
            "command: RedoChangeCommand",
            "command: CreateSnapshotCommand",
            "command: CompareChangesCommand",
            "command: RestoreSnapshotCommand",
            "command: ReviewConflictCommand",
            "data-workbench-history-undo-action=\"help\"",
            "href=\"@HelpHref\"",
            "private const string HelpHref = \"/help\"",
            "data-workbench-history-undo-action=\"conflict\"",
        ],
    },
    {
        "id": "scoped_history_undo_visual_design",
        "path": "Chummer.Blazor/Components/Pages/Preview.razor.css",
        "tokens": [
            ".browser-workbench-history-undo",
            ".browser-workbench-history-undo-copy",
            ".browser-workbench-history-undo-actions",
            "@media (max-width: 720px)",
        ],
    },
    {
        "id": "release_truth_docs",
        "path": "docs/WORKBENCH_RELEASE_SIGNOFF.md",
        "tokens": [
            "workbench history-undo posture",
            "blazor-workbench-history-undo-staged-proof-check.sh",
            "not a hosted or Docker browser execution receipt",
        ],
    },
    {
        "id": "parity_goal_docs",
        "path": "docs/BLAZOR_WEB_CLIENT_PARITY_GOAL.md",
        "tokens": [
            "staged hosted workbench history-undo posture",
            "undo, redo, snapshot, compare, restore, help, and conflict review",
            "not yet claiming actual rollback, diff, portal help runtime, or conflict-resolution parity",
        ],
    },
    {
        "id": "status_utility_reporting",
        "path": "scripts/print_blazor_public_edge_proof_status.py",
        "tokens": [
            "WORKBENCH_HISTORY_UNDO_STAGED_PROOF",
            "workbench_history_undo_staged_status",
            "workbench_history_undo_staged_source_checks",
            "source_alignment_only_not_browser_execution",
        ],
    },
]


def read_text(relative_path: str) -> str:
    return (REPO_ROOT / relative_path).read_text(encoding="utf-8")


def evaluate_check(check: dict) -> dict:
    content = read_text(check["path"])
    missing_tokens = [token for token in check["tokens"] if token not in content]
    return {
        "id": check["id"],
        "path": check["path"],
        "status": "passed" if not missing_tokens else "failed",
        "required_token_count": len(check["tokens"]),
        "missing_tokens": missing_tokens,
    }


def main() -> int:
    evaluated_checks = [evaluate_check(check) for check in CHECKS]
    failures = [check for check in evaluated_checks if check["status"] != "passed"]
    receipt = {
        "contract_name": "chummer6-ui.blazor_workbench_history_undo_staged_proof",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "failed" if failures else "passed",
        "proof_tier": "source_staged_no_browser_execution",
        "route_lane": "promoted_blazor_workbench",
        "expected_routes": ["/blazor/workbench", "/help"],
        "checks": evaluated_checks,
        "failures": failures,
        "notes": [
            "This receipt only proves that Chummer Online and /blazor/workbench compatibility route history-undo source, style, status, and docs agree.",
            "It is not a substitute for hosted Playwright execution proof, Docker self-host proof, rollback execution proof, diff proof, portal help runtime, or conflict-resolution proof.",
            "Do not use this receipt to claim actual rollback, diff, portal help runtime, conflict resolution, persistence, or browser execution parity.",
        ],
    }

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_PATH.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(f"blazor_workbench_history_undo_staged_proof:{receipt['status']} {OUTPUT_PATH}")
    return 0 if not failures else 1


if __name__ == "__main__":
    raise SystemExit(main())
