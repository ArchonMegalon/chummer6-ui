...
restore_local_manifests_from_registry_if_needed() {
  echo "registry manifest fallback is permanently disabled; only this build's staged artifacts may define release truth" >&2
  return 1
}
...
if to_bool "$SCOPE_TO_STAGE_ARTIFACTS"; then
  ...
  echo "registry manifest fallback permanently disabled; only this build's staged artifacts may define release truth"
fi
