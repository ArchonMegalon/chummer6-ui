...
CHECKS = [
    {
        ...
        "tokens": [
            "Browser-client security and access lane",
            "data-workbench-security-access=\"strip\"",
            "Keep hosted access posture visible.",
            "private const string SignInStatusCommand = \"sign_in_status\"",
            "private const string LockWorkspaceCommand = \"lock_workspace\"",
            "private const string ReviewRolesCommand = \"review_roles\"",
            "private const string SessionExpiryCommand = \"session_expiry\"",
            "private const string RotateAccessKeyCommand = \"rotate_access_key\"",
            "private const string AccessAuditCommand = \"access_audit\"",
            "command: SignInStatusCommand",
            "command: LockWorkspaceCommand",
            "command: ReviewRolesCommand",
            "command: SessionExpiryCommand",
            "command: RotateAccessKeyCommand",
            "command: AccessAuditCommand",
...
def main() -> int:
    ...
    receipt = {
        ...
        "failures": failures,
        "notes": [
            "This receipt only proves that Chummer Online and /blazor/workbench compatibility route security-access source, style, status, and docs agree.",
