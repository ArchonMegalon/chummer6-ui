...
4. `CHUMMER_DESKTOP_RELEASE_CHANNEL` is the mainline publication switch: `preview` keeps signing/notarization optional, while `release_candidate` and `public_stable` fail closed unless the workflow emits Windows signing receipts and macOS signing/notarization receipts from the same bundle.
