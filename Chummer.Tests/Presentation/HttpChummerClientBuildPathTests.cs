...
""");
        ...
        CollectionAssert.Contains(projection.ReadinessHighlights.ToArray(), "First-session details: Starter build, restore packet, and campaign lane all point at the same Dockside kickoff.");
        ...
        CollectionAssert.Contains(projection.ReadinessHighlights.ToArray(), "Item audience: My stuff, Campaign stuff, Published stuff");
        CollectionAssert.Contains(projection.ReadinessHighlights.ToArray(), "Item views: My stuff, Campaign stuff, Published stuff stay browseable from the same page.");
        CollectionAssert.Contains(projection.ReadinessHighlights.ToArray(), "Item ownership: Dockside keeps the same reviewed item on the signed-in account path instead of forking a shadow copy.");
        CollectionAssert.Contains(projection.ReadinessHighlights.ToArray(), "Item publication: Preview Ready — Dockside campaign packet is already attached on the publication shelf with shared visibility.");
        CollectionAssert.Contains(projection.ReadinessHighlights.ToArray(), "Item trust: Review Pending — Still limited");
