...
public class DesktopDialogFactoryTests
{
    ...
    public void CreateUiControlDialog_cyberware_add_uses_selection_form_posture()
    {
        ...
        Assert.AreEqual("Bodyware", DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareSelectedBranch"));
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareBrowseGrid"), "Name | Category | Avail | Cost");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareBrowseGrid"), "Cybereyes Rating 4 | Headware | 12 | ¥16,000");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareSelectionTrail"), "Category Path | Cyberware > Core Systems > Bodyware > Wired Reflexes 2");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareSelectionTrail"), "Search Scope | current category only");
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareFilterSummary"), "Category Path | Cyberware > Core Systems > Bodyware");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareLiveRecalc"), "Add Again | Stays open");
        Assert.AreEqual("Add Wired Reflexes 2", dialog.Actions.Single(action => string.Equals(action.Id, "add", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Add & More Wired Reflexes 2", dialog.Actions.Single(action => string.Equals(action.Id, "add_more", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Focus Bodyware", dialog.Actions.Single(action => string.Equals(action.Id, "focus_category", StringComparison.Ordinal)).Label);
    ...
    public void CreateUiControlDialog_gear_add_uses_selection_form_posture()
    {
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiGearSelectionTrail"), "Search Scope | current category only");
        ...
        Assert.AreEqual("Focus Pistols", dialog.Actions.Single(action => string.Equals(action.Id, "focus_category", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Search All Categories", dialog.Actions.Single(action => string.Equals(action.Id, "toggle_search_scope", StringComparison.Ordinal)).Label);
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiGearFilterSummary"), "Category Path | Gear > Firearms > Pistols");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiGearLiveRecalc"), "Add Again | Stays open");
        Assert.AreEqual("Add Ares Predator V", dialog.Actions.Single(action => string.Equals(action.Id, "add", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Add & More Ares Predator V", dialog.Actions.Single(action => string.Equals(action.Id, "add_more", StringComparison.Ordinal)).Label);
    }
    ...
    public void CreateUiControlDialog_combat_reload_uses_dense_utility_posture()
    {
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCombatReloadCommands"), "Reload selected weapon");
        ...
        Assert.AreEqual(DesktopDialogFieldVisualKinds.List, dialog.Fields.Single(field => string.Equals(field.Id, "uiCombatReloadCommands", StringComparison.Ordinal)).VisualKind);
        ...
        Assert.AreEqual("Reload", dialog.Actions.Single(action => string.Equals(action.Id, "apply", StringComparison.Ordinal)).Label);
    }
    ...
    public void CreateUiControlDialog_combat_damage_track_uses_dense_utility_posture()
    {
        ...
        Assert.AreEqual(DesktopDialogFieldVisualKinds.List, dialog.Fields.Single(field => string.Equals(field.Id, "uiDamageTrackCommands", StringComparison.Ordinal)).VisualKind);
    ...
    public void CreateUiControlDialog_vehicle_add_uses_selection_form_posture()
    {
        ...
        Assert.AreEqual("Cars", DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleSelectedBranch"));
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleSelectionTrail"), "Category Path | Vehicles > Ground Vehicles > Cars > Hyundai Shin-Hyung");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleBrowseGrid"), "Name | Category | Avail | Cost");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleBrowseGrid"), "Yamaha Growler | Bikes | 6 | ¥5,000");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleSelectionTrail"), "Search Scope | current category only");
        ...
        Assert.AreEqual("Add Hyundai Shin-Hyung", dialog.Actions.Single(action => string.Equals(action.Id, "add", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Add & More Hyundai Shin-Hyung", dialog.Actions.Single(action => string.Equals(action.Id, "add_more", StringComparison.Ordinal)).Label);
    }
    ...
    public void CreateUiControlDialog_combat_add_weapon_uses_selection_form_posture()
    {
        ...
        Assert.AreEqual("Heavy Pistols", DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponSelectedBranch"));
        Assert.AreEqual("true", DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponSearchInCategoryOnly"));
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponBrowseGrid"), "Name | Category | Avail | Cost");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponBrowseGrid"), "Colt M23 | Heavy Pistols | 5R | ¥750");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponSelectionTrail"), "Category Path | Weapons > Firearms > Heavy Pistols > Colt M23");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponSelectionTrail"), "Search Scope | current category only");
        Assert.AreEqual("Focus Heavy Pistols", dialog.Actions.Single(action => string.Equals(action.Id, "focus_category", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Search All Categories", dialog.Actions.Single(action => string.Equals(action.Id, "toggle_search_scope", StringComparison.Ordinal)).Label);
        ...
        Assert.AreEqual("Add Colt M23", dialog.Actions.Single(action => string.Equals(action.Id, "add", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Add & More Colt M23", dialog.Actions.Single(action => string.Equals(action.Id, "add_more", StringComparison.Ordinal)).Label);
    }
    ...
    public void CreateUiControlDialog_combat_add_armor_uses_selection_form_posture()
    {
        ...
        Assert.AreEqual("Armor", DesktopDialogFieldValueParser.GetValue(dialog, "uiArmorSelectedBranch"));
        Assert.AreEqual("true", DesktopDialogFieldValueParser.GetValue(dialog, "uiArmorSearchInCategoryOnly"));
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiArmorSelectionTrail"), "Category Path | Armor > Protective Wear > Armor > Armor Jacket");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiArmorSelectionTrail"), "Search Scope | current category only");
        ...
        Assert.AreEqual("Add Armor Jacket", dialog.Actions.Single(action => string.Equals(action.Id, "add", StringComparison.Ordinal)).Label);
        Assert.AreEqual("Add & More Armor Jacket", dialog.Actions.Single(action => string.Equals(action.Id, "add_more", StringComparison.Ordinal)).Label);
    }
    ...
    public void RebuildDynamicDialog_cyberware_add_updates_selection_from_filters()
    {
        ...
        Assert.AreEqual("Headware", DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareSelectedBranch"));
        Assert.AreEqual("Core Rulebook p. 455", DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareSource"));
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareCandidateList"), "> Cybereyes Rating 4");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareBrowseGrid"), "Cybereyes Rating 4 | Headware | 12 | ¥17,280");
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareSelectionTrail"), "Category Path | Cyberware > Core Systems > Headware > Cybereyes Rating 4");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiCyberwareCategoryCommands"), "Group | Core Systems");
    ...
    public void RebuildDynamicDialog_gear_add_updates_selection_from_search_and_affordability()
    {
        ...
        Assert.AreEqual("Show All", DesktopDialogFieldValueParser.GetValue(dialog, "uiGearCategory"));
        Assert.AreEqual("Medical", DesktopDialogFieldValueParser.GetValue(dialog, "uiGearSelectedBranch"));
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiGearSelectionTrail"), "Search Scope | current category only");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiGearSelectionTrail"), "Category Path | Gear > General > Medical > Medkit Rating 6");
    ...
    public void RebuildDynamicDialog_weapon_add_updates_selection_and_accessories()
    {
        ...
        Assert.AreEqual("Show All", DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponCategory"));
        Assert.AreEqual("Shotguns", DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponSelectedBranch"));
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponBrowseGrid"), "Defiance T-250 | Shotguns | 4R | ¥450");
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiWeaponSelectionTrail"), "Search Scope | current category only");
        Assert.AreEqual("Focus Shotguns", dialog.Actions.Single(action => string.Equals(action.Id, "focus_category", StringComparison.Ordinal)).Label);
    ...
    public void RebuildDynamicDialog_armor_add_updates_selection_from_branch_filters()
    {
        ...
        Assert.AreEqual("Shields", DesktopDialogFieldValueParser.GetValue(dialog, "uiArmorSelectedBranch"));
    ...
    public void RebuildDynamicDialog_vehicle_add_updates_selection_with_used_vehicle_flow()
    {
        ...
        Assert.AreEqual("Show All", DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleCategory"));
        Assert.AreEqual("Drones", DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleSelectedBranch"));
        ...
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleSelectionTrail"), "Search Scope | current category only");
        StringAssert.Contains(DesktopDialogFieldValueParser.GetValue(dialog, "uiVehicleSelectionTrail"), "Category Path | Vehicles > Drone Platforms > Drones > MCT Fly-Spy");
