# Blazor Workbench Rules-Citation Staged Proof

## Purpose

This staged proof keeps Chummer Online and /blazor/workbench sources aligned around rule/source citation posture for browser output and table handoff workflows.
...
```bash
bash scripts/ai/milestones/blazor-workbench-rules-citation-staged-proof-check.sh
```

The command writes:

```text
.codex-studio/published/BLAZOR_WORKBENCH_RULES_CITATION_STAGED_PROOF.generated.json
```
