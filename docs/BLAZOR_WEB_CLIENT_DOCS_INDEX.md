...
- `scripts/materialize-blazor-pwa-public-edge-proof.py`: live public-edge materializer for the clean `/app` entry plus hosted `/blazor` installable PWA shell, static-cache privacy boundary, offline copy, app-head service-worker registration, clean `/app` entry, static deployed assets, mobile viewport posture
- `scripts/materialize-blazor-play-surface-horizon.py`: aggregate public-status materializer for near-term stabilization, mid-term PWA/session utility, and long-term living-world expansion horizons using current runtime proof, staged browser-lane receipts, and flagship Table Pulse docs without upgrading staged posture into runtime parity
...
- `scripts/print_blazor_public_edge_proof_status.py`: also reports the hosted `/blazor` PWA proof receipt, base URL, public entry URL, proof tier, route lane, and required manifest/service-worker/offline/app-head/clean-public-entry/static-asset/mobile-viewport check ids so operators can confirm the Play shell is installable without treating runner, workspace, API, Black Ledger, heat, or session data as offline cacheable
...
- `scripts/ai/milestones/blazor-play-surface-horizon-check.sh`: milestone-style wrapper for the public browser/PWA play-surface horizon receipt, requiring the three named horizon ids and a passing hosted PWA public-edge status before release-gate consumers can accept it
...
- `.codex-studio/published/BLAZOR_PLAY_SURFACE_HORIZON.generated.json`: published public play-surface horizon receipt for portal/downloads status, separating runtime-proven hosted stabilization from PWA/session staged posture and long-term living-world expansion
...
- `.codex-studio/published/BLAZOR_PWA_PUBLIC_EDGE_PROOF.generated.json`: published hosted `/blazor` PWA install-shell receipt covering manifest, service worker, offline living-world boundary copy, app-head registration, clean `/app` entry, static deployed assets, mobile viewport posture
