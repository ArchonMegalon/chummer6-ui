...
- `docs/BLAZOR_WEB_CLIENT_PARITY_GOAL.md`: main design spec, product posture, Chummer App route alias, parity rules, workflow bar, and required proof standard
...
- `docs/WORKBENCH_RELEASE_SIGNOFF.md`: release-signoff posture for Chummer App routes, proof-compatible workbench routes, and browser workflow proof
...
- `docs/BLAZOR_PUBLIC_EDGE_WORKBENCH_PROOF.md`: contract for hosted `chummer.run` route-entry posture proof, including clean public `/app`, hosted `/blazor/app`, `/blazor/home` roster-first orientation route with the `public_blazor_home_roster_entry` marker, and canonical proof-compatible `/blazor/workbench` route family
...
- `scripts/materialize-blazor-browser-lane-proof-set.py`: generated aggregate receipts also carry notes that `source_staged_release_boundary` is source-policy evidence only and that `MIG-106` through `MIG-109` remain open until refreshed hosted route-entry, hosted execution, Docker self-host, analytics posture, connected-runtime, source-boundary, and aggregate browser-lane receipts prove the browser-client release claim
...
- `docs/BLAZOR_CAREER_SUPPORT_STAGED_PROOF.md`: source-staged contract for the next career/support workflow refresh, including restored calendar continuity, career entry add/edit/delete, runner notes, and move up/down utilities
...
- `Chummer.Presentation/RunnerIntelligence/RunnerIntelligenceSampleFactory.cs`: reusable Increase Initiative scenario and sample fixtures for shared Runner Intelligence calculation posture, including the DI-friendly scenario catalog used by Blazor and Avalonia
- `Chummer.Desktop.Runtime/RunnerIntelligence/DesktopRunnerIntelligenceBridge.cs`: Avalonia-facing bridge that delegates to the shared Runner Intelligence calculator and scenario catalog
...
- `.codex-studio/published/BLAZOR_PUBLIC_EDGE_WORKBENCH_PROOF.generated.json`: published hosted route-entry proof receipt covering clean public `/app`, hosted `/blazor/app`, and proof-compatible `/blazor/workbench`

## Self-Host and Operator Documents

- `docs/BLAZOR_SELF_HOST_RUNBOOK.md`: canonical Docker and operator runbook for Chummer App and the browser workbench lane
...
Hosted `chummer.run` may enable the Rybbit adapter for the Blazor web client, but self-hosted Docker defaults keep analytics disabled with `CHUMMER_ANALYTICS_PROVIDER=none`. Chummer App route telemetry uses the sanitized `chummer_app` route family and route/workflow metadata only, with explicit no session replay and no autocapture posture.

The adapter is limited to route/workflow metadata such as route family, command id, tab id, control id, dialog action id, boolean fixture/workspace presence, and the analytics scope/session replay/autocapture policy fields. It must not emit character names, aliases, owner ids, workspace ids, file names, document contents, XML, payloads, hashes, or generated dossier text.

The Blazor health endpoint exposes the non-secret posture fields `selfHostDefault`, `hostedPublicEdge`, `sensitiveDataPolicy`, `sessionReplayPolicy`, and `autocapturePolicy` so operators can confirm analytics defaults without exposing credentials.
...
- `/blazor/`: stable public entry that should resolve into Chummer App
...
- `/blazor/workbench`: explicit proof-compatible compatibility route for the same promoted browser client
...
- `docs/MIGRATION_BACKLOG.md#browser-client-release-evidence-boundary` keeps `MIG-106` through `MIG-109` open until refreshed hosted route-entry, hosted execution, Docker self-host, analytics posture, connected-runtime, source-boundary, and aggregate browser-lane receipts prove the browser-client release claim.
...
- `docs/BLAZOR_WORKBENCH_RECOVERY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench session recovery contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_HOSTING_PRIVACY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench hosting and privacy posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_COMMAND_PALETTE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench command-palette posture contract, including the same-origin `/help` command. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_DENSITY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench density posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_WORKFLOW_LEDGER_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench workflow-ledger posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_FILE_INTAKE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench file-intake posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_RULES_DATA_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench rules/data posture contract, including the same-origin `/help` rules/data guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_SETTINGS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench settings/preferences posture contract, including the same-origin `/help` settings guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_DIAGNOSTICS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench diagnostics posture contract, including the same-origin `/help` diagnostics guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_CONNECTED_RUNTIME_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench connected-runtime posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_ACCESSIBILITY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench accessibility posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_SECTION_RAIL_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench section-rail posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_MENU_BAR_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench desktop menu-bar posture contract, including the same-origin `/help` menu handoff. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_WORKSPACE_TABS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench workspace-tabs posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_STATUS_BAR_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench status-bar posture contract, including the same-origin `/help` recovery cue. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_INSPECTOR_RAIL_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench inspector-rail posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_DIALOG_STACK_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench dialog-stack posture contract, including the same-origin `/help` recovery continuation. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_CONTEXT_ACTIONS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench context-actions posture contract, including the same-origin `/help` selection guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_SEARCH_FILTER_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench search/filter posture contract, including the same-origin `/help` filter-recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_LAYOUT_PRESETS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench layout-presets posture contract, including the same-origin `/help` layout-recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_ACTIVITY_FEED_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench activity-feed posture contract, including the same-origin `/help` recovery event. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_KEYBOARD_SHORTCUTS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench keyboard-shortcuts posture contract, including the same-origin `/help` keyboard-guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_RESOURCE_METERS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench resource-meters posture contract. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_TREE_TOOLS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench tree-tools posture contract, including the same-origin `/help` dense-list guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_SAVE_SESSION_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench save-session posture contract, including the same-origin `/help` save/session recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_OUTPUT_HANDOFF_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench output-handoff posture contract, including the same-origin `/help` output-recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_VALIDATION_QUEUE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench validation-queue posture contract, including the same-origin `/help` validation-guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_HISTORY_UNDO_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench history-undo posture contract, including the same-origin `/help` rollback guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_SYNC_PRESENCE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench sync-presence posture contract, including the same-origin `/help` sync/offline recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_DATA_PACKS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench data-packs posture contract, including the same-origin `/help` rules/data-pack guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_CHARACTER_LIBRARY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench character-library posture contract, including the same-origin `/help` library/import recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_CAMPAIGN_SESSION_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench campaign-session posture contract, including the same-origin `/help` campaign/table handoff recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_OBSERVABILITY_PRIVACY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench observability-privacy posture contract, including the same-origin `/help` analytics/privacy guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_FIRST_RUN_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench first-run posture contract, including the same-origin `/help` onboarding/setup recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PWA_INSTALL_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench PWA-install posture contract, including the same-origin `/help` install/cache/permissions recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_DOCKER_OPERATOR_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench Docker-operator posture contract, including the same-origin `/help` self-host operator recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_SECURITY_ACCESS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench security-access posture contract, including the same-origin `/help` sign-in/access recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_NOTIFICATIONS_JOBS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench notifications-jobs posture contract, including the same-origin `/help` async-work recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_TOUCH_MOBILE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench touch-mobile posture contract, including the same-origin `/help` touch/mobile recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_NAVIGATION_DEEPLINK_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench navigation-deeplink posture contract, including the same-origin `/help` route/deep-link recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_INLINE_EDITING_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench inline-editing posture contract, including the same-origin `/help` edit guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PERFORMANCE_VIRTUALIZATION_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench performance-virtualization posture contract, including the same-origin `/help` large-sheet performance guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PRINT_LAYOUT_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench print-layout posture contract, including the same-origin `/help` sheet-output guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PORTRAIT_ATTACHMENTS_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench portrait-attachments posture contract, including the same-origin `/help` media/storage guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_WINDOWING_PANES_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench windowing-panes posture contract, including the same-origin `/help` pane/window layout guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_CALCULATION_PROVENANCE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench calculation-provenance posture contract, including the same-origin `/help` calculation-explainability guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_LIFECYCLE_CALENDAR_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench lifecycle-calendar posture contract, including the same-origin `/help` downtime/upkeep guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PROGRESSION_LEDGER_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench progression-ledger posture contract, including the same-origin `/help` advancement-accounting guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_IMPORT_RECONCILE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench import/reconcile posture contract, including the same-origin `/help` existing-runner import guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_COMPARE_MERGE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench compare/merge posture contract, including the same-origin `/help` merge-review guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_RESTORE_CHECKPOINT_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench restore/checkpoint posture contract, including the same-origin `/help` recovery/checkpoint guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_OFFLINE_CACHE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench offline/cache posture contract, including the same-origin `/help` offline/cache guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_SESSION_LOCKING_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench session-locking posture contract, including the same-origin `/help` edit-ownership guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_SHARE_EXPORT_PRIVACY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench share/export privacy posture contract, including the same-origin `/help` private-handoff guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_TABLE_HANDOFF_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench table-handoff posture contract, including the same-origin `/help` table-output guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_RULES_CITATION_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench rules-citation posture contract, including the same-origin `/help` rules-explanation guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_LOCALIZATION_TERMINOLOGY_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench localization/terminology posture contract, including the same-origin `/help` table-language guidance action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_HELP_RECOVERY_GUIDANCE_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench help/recovery guidance posture contract, including the same-origin `/help` handoff action. It is not runtime proof.
- `docs/BLAZOR_WORKBENCH_GM_SCREEN_EXPORT_STAGED_PROOF.md` defines the source-staged Chummer App and proof-compatible workbench GM-screen export posture contract, including the same-origin `/help` GM-screen/export guidance action. It is not runtime proof.
...
- `docs/BLAZOR_PORTAL_INSTALLER_HANDOFF_PROOF.md` defines the source-staged browser-to-portal download/install/support handoff contract. It keeps published artifacts as direct raw downloads while listing Blazor desktop compatibility routes as proof-required handoff rows with stable route metadata, a self-host operator lane for Docker/downloads setup, a release-manifest-backed `/status` page, a marked `/contact` support handoff fallback, a same-origin `/help` handoff guide, the product-first `Explore Chummer Online` portal root hero plus the roster-first primary CTA selector `a.cta[href="/app?command=character_roster"][data-portal-home-action="explore-chummer-online"]`, the `Chummer browser routes` portal root rail with user-facing Open character roster, Open Chummer Online, Open Chummer overview, and the `Get desktop client` label, portal route values centralized through `PortalRoutes.PublicApp`, `PortalRoutes.PublicAppSlash`, `PortalRoutes.PublicAppRoster`, `PortalRoutes.CharacterRosterCommand`, `PortalRoutes.BlazorApp`, `PortalRoutes.BlazorAppSegment`, and `BuildBlazorAppUrl`, portal root rail hover/focus affordances with reduced-motion handling, the same polished Chummer Online slate/amber/mint/blue visual language for portal recovery pages, deep ink/surface contrast, restrained ambient glow, downloads action pills, download route rows, docs shortcut pills, OpenAPI endpoint cards, labelled help/contact/status recovery rails with pill-style keyboard-focus treatment, and help-card plus help/contact/status rail hover/focus affordances with reduced-motion guards with a shared ambient grid texture, mobile-softened grid density, high-contrast portal action affordances, and reduced-motion-safe portal panel reveal. It also keeps same-origin OpenAPI/docs discovery markers for clean public `/app`, hosted `/blazor/app`, `/blazor/home`, `/blazor/`, `/downloads/`, `/downloads/releases.json`, `/downloads/install/{artifactId}`, `/status`, `/contact`, and `/help`, including `data-openapi-chummer-app-route`, `data-openapi-chummer-home-route`, and `data-openapi-blazor-entry-route`. The same `/docs/` explorer is marked with `data-docs-panel="operator-openapi-explorer"`, carries a shortcut rail marked with `data-docs-shortcuts="operator-recovery"` and `aria-describedby="docs-shortcuts-description"`, marks the OpenAPI load state with `data-docs-summary="openapi-load-state"` as a polite status live region, and marks the generated route collection with `data-docs-endpoints="openapi-route-list"` plus `data-docs-endpoint-card="openapi-route"`, `data-docs-endpoint-route`, `data-docs-endpoint-family`, `data-docs-endpoint-methods`, and `data-docs-endpoint-summary` using list/listitem semantics plus direct operator shortcut markers such as `data-docs-action="open-chummer-app"` targeting the clean public `/app?command=character_roster`, `data-docs-action="open-downloads"`, `data-docs-action="open-help"`, `data-docs-action="open-contact"`, and `data-docs-action="open-openapi-json"` so self-host users can recover into the roster workflow, installer, status, help, support, or raw contract routes without reading generated cards first. It includes Avalonia installer handoff and Blazor desktop compatibility installer handoff routes. It is not runtime proof.
...
- `docs/BLAZOR_DOCKER_SELF_HOST_OPERATOR_PROOF.md` defines the source-staged self-hosted Docker operator contract for the portal-backed Blazor browser client, including default-off Rybbit analytics configuration through the sanitized `.env` example with session replay and autocapture disabled for Chummer surfaces. It is source alignment only, not Docker runtime proof.
...
- The refresh-plan example also records `workbench_hosting_privacy_status_visibility` so operators can see the Chummer App and proof-compatible workbench hosting/privacy source-only status lines and default-off Rybbit posture without treating them as Rybbit delivery, hosted route execution, or Docker browser execution.
