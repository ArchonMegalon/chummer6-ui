...
- `docs/BLAZOR_PUBLIC_EDGE_WORKBENCH_PROOF.md`: contract for hosted `chummer.run` route-entry posture proof, including clean public `/app`, clean public `/app?command=character_roster` with the `public_chummer_app_roster_route` marker, roster-first `/blazor/` redirect with the `public_blazor_root_redirect` marker, hosted `/blazor/app`, `/blazor/home` roster-first orientation route with the `public_blazor_home_roster_entry` marker, and canonical proof-compatible `/blazor/workbench` route family
...
- `.codex-studio/published/BLAZOR_PUBLIC_EDGE_WORKBENCH_PROOF.generated.json`: published hosted route-entry proof receipt covering clean public `/app`, clean public `/app?command=character_roster`, hosted `/blazor/app`, and proof-compatible `/blazor/workbench`
...
- `/blazor/`: stable hosted entry that resolves into Chummer Online and immediately opens the roster-first `app?command=character_roster` browser workflow
...
- `/blazor/workbench`: /blazor/workbench compatibility route for the same promoted browser client
...
- `docs/BLAZOR_WORKBENCH_CHARACTER_LIBRARY_STAGED_PROOF.md` defines the source-staged Chummer Online and /blazor/workbench compatibility route Character Roster posture contract, including the same-origin `/help` roster/import recovery action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PERFORMANCE_VIRTUALIZATION_STAGED_PROOF.md` defines the source-staged Chummer Online and /blazor/workbench compatibility route performance-virtualization posture contract, including the same-origin `/help` large-sheet performance guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_PRINT_LAYOUT_STAGED_PROOF.md` defines the source-staged Chummer Online and /blazor/workbench compatibility route print-layout posture contract, including the same-origin `/help` sheet-output guidance action. It is not runtime proof.
...
- `docs/BLAZOR_WORKBENCH_IMPORT_RECONCILE_STAGED_PROOF.md` defines the source-staged Chummer Online and /blazor/workbench compatibility route import/reconcile posture contract, including the same-origin `/help` existing-runner import guidance action. It is not runtime proof.
