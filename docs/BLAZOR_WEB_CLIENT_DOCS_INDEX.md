...
- `scripts/print_blazor_public_edge_proof_status.py`: also reports the hosted `/blazor` PWA proof receipt, base URL, public entry URL, `/pwa` player companion alias, proof tier, route lane, and required manifest/service-worker/offline/app-head/clean-public-entry/player-manifest/mobile-living-world/static-asset/mobile-viewport check ids so operators can confirm the Play shell is installable without treating runner, workspace, API, Black Ledger, heat, or session data as offline cacheable
