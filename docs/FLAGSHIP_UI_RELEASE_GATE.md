...
The default single-runner shell must not waste first-paint width on workspace chrome. The primary familiarity posture is center-first: top menu, immediate toolstrip, visible character-tab workbench, dense section/detail center, and compact status strip. Auxiliary workspace/navigation chrome is allowed only when a real multi-workspace session exists, and it must stay compact when shown.
