...
The home page source keeps that roster-first route derived from `AppRoute` and `CharacterRosterCommand` through `RosterRoute` and `PublicRosterRoute`, so the visible Chummer Online CTA and display pill stay aligned without scattering the startup command string.
