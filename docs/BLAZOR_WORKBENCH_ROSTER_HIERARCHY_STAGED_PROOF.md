...
This staged proof keeps Chummer Online and the proof-compatible Blazor workbench source aligned around the user-facing Character Library and custom character roster hierarchy posture.
...
The roster command opens as `Character Library` and tells users they can group runners into their own folders, drag characters or custom directories through the tree, and keep selected-runner details close without moving watched files until explicitly confirmed.
System library buckets remain valid drop targets for filing runners and links, but they are not draggable source directories; only user-created custom directories can be nested, renamed, reordered, or deleted. The Blazor tree styles system rows as bucket targets, custom rows as folder targets, runner rows as direct character items, and watched-file or shortcut rows as linked library references.
The Blazor dialog source keeps roster field ids, folder scopes, and drag/drop mutation command ids centralized as DialogHost-local constants, including `RosterCustomFoldersFieldId`, `RosterHierarchyStatusFieldId`, `RosterSystemFolderScope`, `RosterCustomFolderScope`, `RosterMoveRunnerToGroupCommand`, and `RosterReorderTreeCommand`, so drag/drop targeting, source-kind detection, folder-scope selectors, and bridge command ids do not drift through repeated string literals.

Visible management actions use Library vocabulary for the same workflow, including `Open Library Folder`, `Create Library Folder`, `Configure Library Folder`, `Create Library Directory`, `Rename Library Directory`, `Delete Library Directory`, and `Library Entries`, while internal action IDs remain roster-oriented for compatibility.

Command-list and status text follows the same wording, including `Create custom library directory`, `Undo last library move`, `Save runner to library folder`, and `configure a library folder first`.
...
The app shell labels the direct roster shortcut as `Character Library` with the visible action summary `Find, group, and organize existing runners.` so search/filter and startup surfaces use the same user-facing vocabulary as the dialog.
