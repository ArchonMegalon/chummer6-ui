...
Acceptance criteria: shell state includes command surfaces, menu state, navigation state, status/notice/error, and active dossier context.
...
- [x] `MIG-020` Evolve workspace session state to explicitly track active dossier and recent-dossier ordering rules.
...
Progress: Blazor `MdiStrip` and `OpenWorkspaceTree` bind to `State.Session` open/active dossier state with shared presenter-driven switch/close flows.
...
- [ ] `MIG-106` Raise `Chummer.Blazor` from browser preview to desktop-equivalent startup/workbench posture.
Acceptance criteria: the browser head presents the same practical first-launch commands and core workbench shell posture as Avalonia, with explicit browser-safe affordances for open/import/new/save/export/print flows.
Progress: `/app` now provides the clean public Chummer Online route and `/blazor/` resolves into Chummer Online, with `/blazor/app` as the hosted app path, `/blazor/home` as the product/orientation page, and `/blazor/workbench` retained as the explicit /blazor/workbench compatibility route backed by the same shell as `/blazor/preview`. Public Chummer Online and /blazor/workbench compatibility routes now expose practical startup cards for new/open/import, restored-session build-lab continuation, browser-safe save/export/print continuations, state-backed recent-dossier resume links driven by shared session state, restored-session continuation lanes that reopen the active dossier directly on contacts/profile/rules/gear/advanced tabs, restored-session action continuations that can open live add-contact, add-complex-form, add-initiation, add-cyberware, and add-spell dialogs after resume, restored-session committed action continuations that can add and keep a contact, initiation, cyberware, and spell with visible row changes, and restored-session result continuations that reopen the active dossier directly into browser download/export/print flows, including explicit `/blazor/workbench?workspace={id}` restoration. `/blazor/preview` remains the denser proof shelf for startup deep links, seeded runner continuity routes, and seeded browser result-state routes for `save_character`, `save_character_as`, `print_character`, and `export_character`. The design contract for this lane is now explicitly captured in `docs/BLAZOR_WEB_CLIENT_PARITY_GOAL.md`: `/app` is the clean public Chummer Online route, `/blazor/app` is the hosted app path, `/blazor/home` is the orientation route, `/blazor/workbench` is the /blazor/workbench compatibility route, and `/blazor/preview` remains a proof/support route. Remaining work is to graduate more workflow families from proof coverage into the product-shaped Chummer Online experience.

- [ ] `MIG-107` Add browser-specific parity proof for core dialog, section, and runner workflows.
Acceptance criteria: Playwright and component/unit coverage prove new-character, origin-dossier, runner dossier navigation, section quick actions, and dialog families through `Chummer.Portal` under `/blazor/`, with desktop and mobile viewport evidence.
Progress: browser proof now stands on its own for startup command deep links, including hosted/public `new_character`, `new_character_origin`, `character_roster`, `master_index`, `open_character`, `open_for_printing`, and `open_for_export`, plus restored-session build-lab continuation, seeded Build Lab/Rules/Contacts/Complex Forms proof lanes, desktop/mobile startup posture, state-backed recent-dossier resume links, restored-session continuation lanes, multiple restored-session action continuations, multiple restored-session committed action continuations, restored-session result continuations, explicit `/blazor/workbench?workspace={id}` restoration, and seeded save/save-as/print/export result-state routes under both direct Blazor workbench/preview routes and portal-backed `/blazor/*`. Hosted public-edge route-entry proof is now published separately for `https://chummer.run/blazor/`, including route-shape evidence for result continuations, action continuations, and committed action continuations. The hosted execution contract now also promotes `character_roster` and `master_index` beyond title-only startup entry by requiring dense browser-visible utility markers for those tool surfaces, adds a dedicated origin/rules continuity family for the `Origin Dossier` wizard and restored `tab-rules` browser environment, promotes restored `tab-create` into a build-lab continuity family with visible planner/progression/hand-off markers, adds combat-lane `combat_add_weapon` execution as a dense combat utility family, adds skills-lane `skill_add` execution as a dense skills utility family, adds gear-lane `vehicle_add`, `vehicle_mod_add`, and `drug_add` execution as dense runner dossier editing families, adds qualities-lane `quality_add` plus `quality_delete` execution as dense non-gear utility and delete/recovery families, adds magician-lane `spell_add` plus `magic_delete` execution as magic utility and delete/recovery families, adds cyberware-lane `cyberware_add`, `cyberware_edit`, and `cyberware_delete` execution as dense cyberware utility families, adds contacts-lane `contact_connection` execution as a compact edit family, adds gear-lane `vehicle_edit` plus `vehicle_delete` execution as edit/recovery runner utility families, and adds contacts-lane `contact_remove` plus `contact_edit` execution as cross-lane recovery/edit families. Remaining work is breadth across more runner dossier workflows and dialog families before public desktop-equivalent claims are justified.
Progress: staged hosted Playwright coverage now adds `promoted_career_log_continuity` for `/blazor/workbench?workspace=<id>&tab=tab-calendar`, `promoted_career_entry_execution` for `/blazor/workbench?workspace=<id>&tab=tab-calendar&control=create_entry`, `promoted_career_entry_committed_execution` for the matching `dialog_action=add` route, `promoted_career_entry_edit_execution` and `promoted_career_entry_delete_execution` for the career log edit/remove lifecycle, committed edit/delete assertions through `promoted_career_entry_edit_committed_execution` and `promoted_career_entry_delete_committed_execution`, runner-notes open/save coverage through `promoted_runner_notes_execution` and `promoted_runner_notes_committed_execution`, plus career-entry list reordering through `promoted_career_entry_reorder_execution`. The Chummer Online and /blazor/workbench compatibility route restored-continuations and restored-actions cards now expose those career/support affordances as visible product paths. This widens the next proof refresh toward the career log / support workflow family without yet claiming that current published hosted execution receipt contains the new families.
Progress: Docker self-host Playwright coverage is now staged for the matching career/support section, add/edit/delete action, runner-notes action, move up/down list utilities, and committed-result routes so the next self-host receipt refresh can prove the same workflow family locally without borrowing hosted evidence. The self-host receipt metadata is also staged to declare those career/support route shapes in `BLAZOR_SELF_HOST_WORKBENCH_PROOF.generated.json` on the next run.
Progress: added `scripts/ai/milestones/blazor-career-support-staged-proof-check.sh` as a source-structural pre-refresh guard for the staged career/support route family. It checks product affordances, hosted route-entry probing, hosted execution probing, Docker self-host probing, self-host receipt metadata, combined status reporting, and docs without claiming browser execution proof.
Progress: staged the next restored browser workflow family for identity/SIN/license utility posture under `tab-info`, including browser-visible add/edit/remove utility dialogs, workbench affordances, hosted route-entry shapes, hosted/self-host Playwright probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: staged the restored combat support browser workflow family under `tab-combat`, including add-armor, reload, and damage-track utility routes, product affordances, hosted/self-host runner probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: staged the restored skill maintenance browser workflow family under `tab-skills`, including specialization, removal, and group-edit utility routes, product affordances, hosted/self-host runner probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: staged the restored magic/resonance support browser workflow family across adept, magician, critter, and technomancer lanes, including adept power, spirit, critter power, and Matrix program utility routes, product affordances, hosted/self-host runner probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: staged the restored generic gear maintenance browser workflow family under `tab-gear`, including add, edit, and remove utility routes, product affordances, hosted/self-host runner probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: staged the restored source/gear utility browser workflow family across profile and gear lanes, including source viewing, gear source, gear mount, and free/paid cost utility routes, product affordances, hosted/self-host runner probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: staged the restored magic cleanup utility browser workflow family across magician and gear lanes, including magic add, spirit binding, magic source, and drug-removal utility routes, product affordances, hosted/self-host runner probes, self-host metadata routes, status reporting, and a source-only pre-refresh guard. This is not yet current hosted or Docker execution receipt evidence.
Progress: added a source-only legacy UI-control coverage guard that maps every known `LegacyUiControlCatalog` control into either current hosted execution baseline coverage or a staged browser route family. This is a breadth guard for the next proof refresh, not browser execution evidence.
Progress: added a source-staged proof-set aggregate that runs the staged source materializers and summarizes their receipt status, route counts, and source-check counts separately from hosted execution and Docker self-host proof. This is operator visibility for staged breadth, not runtime evidence.

- [ ] `MIG-108` Close hosted and self-hosted web-client route/runtime proof.
Acceptance criteria: `chummer.run`-style hosted routing and Docker self-hosted routing both prove reload/deep-link/reconnect behavior, owner/session propagation, downloads handoff, and optional AI/session/coach forwarding through `Chummer.Portal`.
Progress: Docker self-host route proof exists for the portal-backed browser-client lane, and hosted public-edge route-entry proof now exists separately for `https://chummer.run/blazor/`. A dedicated hosted execution-proof contract, runner scaffold, verifier, example receipt, and published passing hosted receipt are now in place so `chummer.run` is promoted on browser workflow execution evidence rather than route posture alone, including hosted startup execution for `new_character_origin`, `character_roster`, and `master_index` in addition to the existing promoted commands.
Progress: self-hosted route/runtime proof now covers `/blazor/` landing into Chummer Online, explicit `/blazor/app`, explicit `/blazor/home`, explicit `/blazor/workbench`, and current restored `/blazor/workbench?workspace=<promoted-workspace-id>` routes, startup deep links, state-backed recent-dossier resume posture, restored-session build-lab/section/action continuation lanes, multiple restored-session committed action continuations, restored-session result continuations, seeded save/save-as/print/export result states, downloads handoff, and owner-context propagation through the portal edge. Remaining work is to widen optional AI/session/coach forwarding evidence when those upstream lanes are active.
Progress: hosted public-edge smoke now targets the browser-client posture directly by expecting clean `/app`, `/blazor/` to resolve into Chummer Online, and hosted `/blazor/health`, `/blazor/home`, `/blazor/app`, `/blazor/workbench`, workspace restore route shape, and startup deep-link route shape. Remaining work is to back those hosted route probes with full browser workflow/runtime proof instead of route-entry evidence alone.
Progress: hosted public-edge evidence is now published separately as `BLAZOR_PUBLIC_EDGE_WORKBENCH_PROOF.generated.json`, so the `chummer.run` browser lane has its own receipt for `/blazor` root redirect, workbench route shape, workspace restore route shape, startup deep-link route shape, and hosted route-health truth instead of hiding inside the desktop/download blocker receipt.
Progress: hosted public-edge browser proof now also records route-shape coverage for resumed result continuations (`save_character_as`, `export_character`, `print_character`) plus resumed action/committed-action entry routes (`contact_add`, `contact_add&dialog_action=add`) and staged career/support routes (`tab-calendar`, `tab-calendar&control=create_entry`, `tab-calendar&control=create_entry&dialog_action=add`, `tab-calendar&control=edit_entry`, `tab-calendar&control=edit_entry&dialog_action=apply`, `tab-calendar&control=delete_entry`, `tab-calendar&control=delete_entry&dialog_action=delete`, `tab-info&control=open_notes`, `tab-info&control=open_notes&dialog_action=save`, `tab-calendar&control=move_up`, `tab-calendar&control=move_down`). This is still route-entry truth, not hosted workflow-execution proof.
Progress target: the next hosted-lane promotion step is not more route-shape breadth but a separate execution-proof lane as defined in `docs/BLAZOR_PUBLIC_EDGE_EXECUTION_PROOF.md`.
Progress note: the hosted execution lane now has a published passing receipt at `.codex-studio/published/BLAZOR_PUBLIC_EDGE_EXECUTION_PROOF.generated.json`, so hosted browser workflow execution is explicit in the published artifact set rather than implied only by self-host or route-entry evidence. The canonical verifier for that receipt now rejects empty workflow-family labels and requires explicit per-family route/assertion/status checks on the promoted `/blazor/workbench` lane before a passing hosted receipt is accepted. The next published receipt refresh should also carry the new dense-tool execution family for hosted `character_roster` and `master_index`, the new origin/rules continuity family, the new build-lab continuity family, the new combat weapon-selection family, the new skills selection family, the new vehicle-selection, vehicle-mod-selection, drug-selection, quality-selection, quality-delete, spell-selection, magic-delete, cyberware-selection, cyberware-edit, and cyberware-delete execution families, the new contact-connection execution family, the new vehicle edit/delete execution families, and the new contact delete/edit execution families.
Progress: Docker self-host proof now covers reload-safe `/blazor/` routing, first-class `/blazor/app`, `/blazor/home`, and `/blazor/workbench` routes, startup command deep links, seeded save/save-as/print/export result-state routes, owner-context portal home, downloads shelf, and route/runtime receipts via `BLAZOR_SELF_HOST_WORKBENCH_PROOF.generated.json`. Remaining work is broader runtime forwarding evidence when session/coach/AI lanes are active.
Progress target: keep hosted and self-hosted proof distinct so local Docker success cannot be mistaken for public-edge readiness.

- [ ] `MIG-109` Define honest browser parity boundaries for native-only desktop behaviors.
Acceptance criteria: for print/export/install/update/support flows that differ materially between native and browser delivery, the browser head provides the closest useful workflow and release evidence documents the difference explicitly.
Progress target: remove ambiguous fallback language that hides platform differences while preserving a desktop-equivalent user workflow where the browser can honestly support it.

### Browser-client release evidence boundary

The MIG-106 through MIG-109 backlog items target the same Blazor web-client goal tracked in `docs/BLAZOR_WEB_CLIENT_PARITY_GOAL.md#release-evidence-boundary`, `docs/BLAZOR_RUNTIME_PROOF_REFRESH_PLAN.md#extended-goal-refresh-gates`, and `docs/WORKBENCH_RELEASE_SIGNOFF.md#extended-goal-release-blockers`.

Source-staged workflow breadth, source-plan receipts, and source-calculation receipts keep the implementation and proof refresh aligned, but they do not close MIG-106 through MIG-109 by themselves. Closing those items requires refreshed hosted route-entry, hosted execution, Docker self-host, analytics posture, connected-runtime, source-boundary, and aggregate browser-lane receipts, with `/app` kept as the clean public Chummer Online route, `/blazor/app` kept as the hosted app path, and `/blazor/workbench` kept as the proof-compatible execution lane until the proof verifiers are deliberately migrated together.
