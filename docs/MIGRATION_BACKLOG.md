...
Progress: hosted public-edge browser proof now also records route-shape coverage for resumed result continuations (`save_character_as`, `export_character`, `print_character`) plus resumed action/committed-action entry routes (`contact_add`, `contact_add&dialog_action=add`) and staged career/support routes (`tab-calendar`, `tab-calendar&control=create_entry`, `tab-calendar&control=create_entry&dialog_action=add`). This is still route-entry truth, not hosted workflow-execution proof.
