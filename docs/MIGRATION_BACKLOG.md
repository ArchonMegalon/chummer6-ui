...
Progress: staged hosted Playwright coverage now adds `promoted_career_log_continuity` for `/blazor/workbench?workspace=<id>&tab=tab-calendar`, `promoted_career_entry_execution` for `/blazor/workbench?workspace=<id>&tab=tab-calendar&control=create_entry`, `promoted_career_entry_committed_execution` for the matching `dialog_action=add` route, plus `promoted_career_entry_edit_execution` and `promoted_career_entry_delete_execution` for the career log edit/remove lifecycle. The promoted workbench restored-continuations and restored-actions cards now expose those career/support affordances as visible product paths. This widens the next proof refresh toward the career log / support workflow family without yet claiming that current published hosted execution receipt contains the new families.
Progress: Docker self-host Playwright coverage is now staged for the matching career/support section, add/edit/delete action, and committed-result routes so the next self-host receipt refresh can prove the same workflow family locally without borrowing hosted evidence.
