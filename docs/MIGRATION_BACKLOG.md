...
Progress: staged hosted Playwright coverage now adds `promoted_career_entry_execution` for `/blazor/workbench?workspace=<id>&tab=tab-calendar&control=create_entry`, widening the next proof refresh toward the career log / support workflow family without yet claiming that current published hosted execution receipt contains the new family.
