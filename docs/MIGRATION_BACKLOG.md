...
Progress: staged hosted Playwright coverage now adds `promoted_career_entry_execution` for `/blazor/workbench?workspace=<id>&tab=tab-calendar&control=create_entry` and `promoted_career_entry_committed_execution` for the matching `dialog_action=add` route. The promoted workbench restored-actions card now exposes both career-entry continuations as visible product affordances. This widens the next proof refresh toward the career log / support workflow family without yet claiming that current published hosted execution receipt contains the new families.
...
Progress: hosted public-edge browser proof now also records route-shape coverage for resumed result continuations (`save_character_as`, `export_character`, `print_character`) plus resumed action/committed-action entry routes (`contact_add`, `contact_add&dialog_action=add`). This is still route-entry truth, not hosted workflow-execution proof.
