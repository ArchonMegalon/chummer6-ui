...
This document defines the source-staged proof contract for account, owner-context, and support handoff from Chummer Online and the promoted Chummer Online browser client.

Chummer Online should behave like another Chummer client, so account recovery, support, status, help, and work-continuation routes must stay part of the same portal product shape instead of forcing users into disconnected pages.
...
- `/blazor/workbench` remains the /blazor/workbench compatibility route for the same promoted Chummer Online client.
