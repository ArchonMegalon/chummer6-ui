...
The browser-hosted workbench is a separate delivery lane from native desktop packaging. In that lane, `/blazor/` is the stable public entry, `/blazor/workbench` is the promoted product-shaped browser client, and `/blazor/preview` remains a supporting proof surface rather than the primary user promise. For Docker self-hosting of `Chummer.Blazor` behind `Chummer.Portal`, use [BLAZOR_SELF_HOST_RUNBOOK.md](/docker/chummercomplete/chummer-presentation/docs/BLAZOR_SELF_HOST_RUNBOOK.md). For the broader browser-client design and parity bar, use [BLAZOR_WEB_CLIENT_PARITY_GOAL.md](/docker/chummercomplete/chummer-presentation/docs/BLAZOR_WEB_CLIENT_PARITY_GOAL.md).
