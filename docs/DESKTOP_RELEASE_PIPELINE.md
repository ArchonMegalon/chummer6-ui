...
The browser-hosted workbench is a separate delivery lane from native desktop packaging. In that lane, `/app` is the clean public Chummer App route, `/blazor/` is the stable hosted Blazor entry into Chummer App, `/blazor/app` is the hosted app path, `/blazor/home` is the product/orientation page, `/blazor/workbench` remains the explicit proof-compatible compatibility route for the same promoted browser client, and `/blazor/preview` remains a supporting proof surface rather than the primary user promise. For Docker self-hosting of `Chummer.Blazor` behind `Chummer.Portal`, use [BLAZOR_SELF_HOST_RUNBOOK.md](BLAZOR_SELF_HOST_RUNBOOK.md). For the broader browser-client design and parity bar, use [BLAZOR_WEB_CLIENT_PARITY_GOAL.md](BLAZOR_WEB_CLIENT_PARITY_GOAL.md).
