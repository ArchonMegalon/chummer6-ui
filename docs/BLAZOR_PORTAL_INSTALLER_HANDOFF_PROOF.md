...
- Desktop install/download handoff remains same-origin through `Chummer.Portal`. Published artifact rows live in a labelled `data-download-list="published-artifacts"` region. The section also exposes a visible `data-download-count` summary. Its empty state is explicitly marked with `data-download-empty="true"`. Published artifact rows keep the visible link on the actual artifact download so anonymous users can install without a login loop. Route probes assert the rendered rows expose `data-download-action`, `data-download-install-route`, `data-download-raw-url`, `data-download-link-mode="raw-url"`, `data-download-platform`, `data-download-platform-label`, and visible `direct download` text. They preserve the raw manifest URL in `data-download-raw-url` and expose the matching install handoff route as metadata for installer-claim and support flows. Published rows also render the artifact id visibly, or `artifact id pending` when absent. Rows expose `data-download-link-mode="raw-url"` and visible `direct download` text so probes can confirm the user-facing link is not a login-gated handoff wrapper.
