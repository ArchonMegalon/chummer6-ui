...
- staged hosted career-log posture for restored `tab-calendar`, `tab-calendar&control=create_entry`, and `dialog_action=add`, with the restored-continuations and restored-actions cards exposing career/support utility lanes and the next hosted execution refresh set to assert section landing, compact list/detail editor, and visible commit result
...
- separate self-host receipt proof for portal-backed `/blazor/workbench` and `/blazor/preview` routes under Docker, with the next refresh staged to include the same career/support section, action, and committed-result lane as the hosted proof runner
