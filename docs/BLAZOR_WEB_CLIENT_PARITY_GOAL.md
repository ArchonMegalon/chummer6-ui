...
- staged hosted workbench compare/merge posture for diff view, conflict choice, source trace, dry run, apply merge, and rollback plan actions, keeping migration review visible on the promoted route while not yet claiming diff calculation, merge mutation, conflict resolution, rollback, or import persistence parity
