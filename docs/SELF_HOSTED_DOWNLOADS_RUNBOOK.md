...
5. Startup-smoke receipts copied during publish must be fresh (default max age: `86400` seconds). Override with `CHUMMER_PUBLISH_STARTUP_SMOKE_MAX_AGE_SECONDS` (or shared `CHUMMER_DESKTOP_STARTUP_SMOKE_MAX_AGE_SECONDS`) only when the release lane explicitly approves a longer evidence window.
