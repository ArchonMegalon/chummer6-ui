...
This is route-entry evidence only. It is not workflow execution proof. The refreshed hosted route-entry receipt must include the `public_chummer_app_route` marker for clean `/app` and the `public_blazor_home_roster_entry` marker so `/blazor/home` proves the roster-first home route pill alongside the hosted `/blazor/app` path.
...
- regenerate affected source-staged, source-plan, source-calculation, hosted runtime, Docker self-host, and aggregate browser-lane receipts after the `/app` route, Chummer App marker, Rybbit privacy, portal handoff, and theme-contract source changes land
