...
Generated at: 2026-06-18T03:35:42.399268Z
...
- Visual parity yes/no: 83/1
- Behavioral parity yes/no: 83/1
- Chummer6-only extras present: 0
- Removable extras present: 0
- Active/productive/nonproductive shard runs: 3/3/0
...
- [HIGH] readiness_gap: Flagship readiness still contains open coverage keys outside the surface-level desktop parity matrix. desktop_client: Executable desktop exit gate proof is missing or not passed. Desktop shell/install/support liveliness must be proven from shipped artifacts., Executable gate blocker: Release channel rolloutState is not a recognized registry rollout posture for desktop install media: stable., Executable gate blocker: Release channel does not publish desktop install media for required platform 'macos'., Executable gate blocker: Release channel desktopTupleCoverage requiredDesktopPlatforms is missing required policy platform(s): macos., Executable gate blocker: Release channel rolloutState must be local_docker_preview/promoted_preview/release_candidate/public_stable when status is publishable and required desktop tuple coverage is complete., Executable gate blocker: Windows desktop exit gate receipt channelId/channel does not match release channel channelId., Executable gate blocker: Windows desktop exit gate receipt checks.release_channel_id does not match release channel channelId., Executable gate blocker: Windows desktop exit gate receipt checks.release_channel_version does not match release channel version., Executable gate blocker: Windows desktop exit gate receipt releaseVersion/version does not match release channel version., Executable gate blocker: Windows gate embedded release_channel_windows_artifact channelId/channel does not match promoted release channel., Executable gate blocker: Windows gate embedded release_channel_windows_artifact version/releaseVersion does not match promoted release channel version., Executable gate blocker: Windows gate embedded release_channel_windows_artifact sha256 does not match promoted release channel., Executable gate blocker: Windows gate embedded release_channel_windows_artifact sizeBytes does not match promoted release channel., Executable gate blocker: Windows startup smoke receipt status is not passing for promoted installer bytes., Executable gate blocker: Windows startup smoke receipt readyCheckpoint is not pre_ui_event_loop for promoted installer bytes., Executable gate blocker: Windows startup smoke receipt operatingSystem is missing for promoted installer bytes., Executable gate blocker: linux desktop exit gate proof for avalonia:linux-x64 is stale (477630s old)., Executable gate blocker: Linux desktop exit gate receipt head channelId/channel does not match release channel for promoted head 'avalonia'., Executable gate blocker: Linux desktop exit gate receipt checks.release_channel_id does not match release channel channelId for promoted head 'avalonia'., Executable gate blocker: Linux desktop exit gate receipt checks.release_channel_version does not match release channel version for promoted head 'avalonia'., Executable gate blocker: Linux desktop exit gate receipt releaseVersion/version does not match release channel version for promoted head 'avalonia'., Executable gate blocker: Linux installer startup smoke receipt channelId does not match release channel for promoted head 'avalonia'., Executable gate blocker: Linux installer startup smoke receipt version does not match release channel version for promoted head 'avalonia'., Executable gate blocker: Linux gate embedded release_channel_linux_artifact channelId/channel does not match promoted release channel., Executable gate blocker: Linux gate embedded release_channel_linux_artifact version/releaseVersion does not match promoted release channel version., Executable gate blocker: Linux gate embedded release_channel_linux_artifact sha256 does not match promoted release channel., Executable gate blocker: Linux gate embedded release_channel_linux_artifact sizeBytes does not match promoted release channel., Executable gate blocker: Linux installer startup smoke receipt artifactDigest does not match promoted release-channel artifact bytes for head 'avalonia'., Executable gate blocker: Stale passing platform gate receipts exist for non-promoted desktop tuples: macos:avalonia:osx-arm64., Executable gate reports stale passing platform gate receipts for non-promoted desktop tuples: macos:avalonia:osx-arm64., Release channel publishes Linux installer media, but executable-gate evidence is missing passing Linux startup-smoke tuple proof., Release channel publishes Windows installer media, but executable-gate evidence is missing passing Windows startup-smoke tuple proof., Chummer5A UI element parity audit still has unresolved release-blocking rows: Dice Initiative And Table Utilities (no/no)., Chummer5A UI element parity audit still reports open parity gaps: visual_no_count=1, behavioral_no_count=1.
...
| Hero Lab importer route | legacy_source_anchor | yes | yes | yes | no | Catalog, dialog-factory, and dialog-coordinator proofs directly cover the Hero Lab importer route. |
