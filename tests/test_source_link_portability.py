...
@pytest.fixture
def portal_lookup(tmp_path):
    """Real temporary paths; no portal, .NET, browser or package installation."""
    path = SCRIPT.parents[2] / "tests/test_portal_release_shelf_runtime.py"
    spec = importlib.util.spec_from_file_location("portal_lookup_test_source", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    module.REPO_ROOT = tmp_path / "workspace" / "ui"
    (module.REPO_ROOT / "scripts").mkdir(parents=True)
    return module


def test_portal_node_path_preserves_explicit_lists_and_existing_candidate_order(portal_lookup, tmp_path):
    environment = {
        "NODE_PATH": os.pathsep.join([str(tmp_path / "first path"), str(tmp_path / "second")]),
        "CHUMMER_PLAYWRIGHT_NODE_PATH": str(tmp_path / "explicit"),
        "CHUMMER_PLAYWRIGHT_ROOT": str(tmp_path / "playwright root"),
    }
    before = environment.copy()
    expected = [
        environment["NODE_PATH"], environment["CHUMMER_PLAYWRIGHT_NODE_PATH"],
        str(tmp_path / "playwright root" / "node_modules"),
        str(portal_lookup.REPO_ROOT.parent / "chummer.run-services" / "node_modules"),
        str(portal_lookup.REPO_ROOT.parent / "node_modules"),
        str(portal_lookup.REPO_ROOT / "scripts" / "node_modules"),
    ]
    assert portal_lookup._portal_playwright_node_path(environment) == os.pathsep.join(expected)
    assert environment == before
    assert not (portal_lookup.REPO_ROOT / "chummer.run-services").exists()


@pytest.mark.parametrize("location", ["explicit-list", "explicit", "root", "sibling", "workspace", "scripts"])
def test_portal_lookup_resolves_local_fixture_package_without_host_alias(portal_lookup, tmp_path, location):
    # Exercise the real local Node resolver against inert fixture package bytes.
    # require.resolve must not execute this fake package or start a browser.
    node = shutil.which("node")
    assert node is not None, "local Node resolver is required; no download or replacement is attempted"
    paths = {
        "explicit-list": tmp_path / "list modules",
        "explicit": tmp_path / "explicit modules",
        "root": tmp_path / "explicit root" / "node_modules",
        "sibling": portal_lookup.REPO_ROOT.parent / "chummer.run-services" / "node_modules",
        "workspace": portal_lookup.REPO_ROOT.parent / "node_modules",
        "scripts": portal_lookup.REPO_ROOT / "scripts" / "node_modules",
    }
    # Earlier candidates are deliberately absent. Node must preserve path lists
    # and continue to the existing explicit/sibling roots without a nested alias.
    environment = {
        "NODE_PATH": os.pathsep.join([str(tmp_path / "missing"), str(paths["explicit-list"])]),
        "CHUMMER_PLAYWRIGHT_NODE_PATH": str(paths["explicit"]),
        "CHUMMER_PLAYWRIGHT_ROOT": str(paths["root"].parent),
    }
    package = paths[location] / "playwright"
    package.mkdir(parents=True)
    entry = package / "index.js"
    entry.write_text("throw new Error('fixture package must not execute');\n")
    result = subprocess.run(
        [node, "-e", "process.stdout.write(require.resolve('playwright'))"],
        cwd=portal_lookup.REPO_ROOT / "scripts", timeout=10, capture_output=True, text=True,
        env={"PATH": os.defpath, "NODE_PATH": portal_lookup._portal_playwright_node_path(environment)},
        check=False,
    )
    assert result.returncode == 0, result.stderr
    assert Path(result.stdout) == entry
    assert not (portal_lookup.REPO_ROOT / "chummer.run-services").exists()


@pytest.mark.parametrize("override", [None, "", "../explicit Core/Chummer.Contracts.csproj"])
def test_portal_command_uses_explicit_or_physical_sibling_core_without_dotnet(
    portal_lookup, monkeypatch, override,
):
    if override is None:
        monkeypatch.delenv("CHUMMER_LOCAL_CONTRACTS_PROJECT", raising=False)
    else:
        monkeypatch.setenv("CHUMMER_LOCAL_CONTRACTS_PROJECT", override)
    monkeypatch.setattr(portal_lookup, "_find_free_port", lambda: 12345)
    calls = []
    def capture(command, **kwargs):
        calls.append((command, kwargs))
        raise RuntimeError("modeled launch boundary; no dotnet executed")
    monkeypatch.setattr(portal_lookup.subprocess, "Popen", capture)
    with pytest.raises(RuntimeError, match="modeled launch boundary"):
        with portal_lookup._running_portal():
            pytest.fail("portal must not start")
    command, options = calls[0]
    expected = override or str(portal_lookup.REPO_ROOT.parent / "chummer-core-engine" /
                               "Chummer.Contracts" / "Chummer.Contracts.csproj")
    assert "-p:ChummerLocalContractsProject=" + expected in command
    assert "-p:ChummerUseLocalCompatibilityTree=true" in command
    assert options["cwd"] == portal_lookup.REPO_ROOT
    assert not (portal_lookup.REPO_ROOT / "chummer-core-engine").exists()


@pytest.mark.parametrize("failure", [
    "missing-node", "missing-module", "injected",
    "DOWNLOADS_POLISH_JOURNEY.generated.json", "downloads-desktop.png", "downloads-mobile.png",
])
def test_portal_lookup_keeps_existing_failure_and_cleanup_assertions(
    portal_lookup, monkeypatch, tmp_path, failure,
):
    monkeypatch.setattr(portal_lookup, "_running_portal", lambda: nullcontext("http://unused.invalid"))
    def run(command, **kwargs):
        assert command[0] == "node"
        assert kwargs["timeout"] == 120 and kwargs["check"] is False
        assert kwargs["env"]["CHUMMER_PORTAL_DOWNLOADS_POLISH_TEST_FAIL_AFTER_VIEWPORT"] == "desktop"
        assert str(portal_lookup.REPO_ROOT.parent / "chummer.run-services" / "node_modules") in (
            kwargs["env"]["NODE_PATH"].split(os.pathsep)
        )
        if failure == "missing-node":
            raise FileNotFoundError("modeled missing node")
        if failure.endswith((".json", ".png")):
            receipt_dir = Path(kwargs["env"]["CHUMMER_PORTAL_DOWNLOADS_POLISH_RECEIPT_DIR"])
            receipt_dir.mkdir()
            (receipt_dir / failure).write_text("modeled leftover")
        stderr = ("Cannot find module 'playwright'" if failure == "missing-module" else
                  "Injected downloads polish failure after desktop evidence.")
        return SimpleNamespace(returncode=1, stdout="", stderr=stderr)
    monkeypatch.setattr(portal_lookup.subprocess, "run", run)
    if failure == "injected":
        portal_lookup.test_downloads_playwright_failure_removes_partial_pass_receipts(tmp_path)
    else:
        expected_error = FileNotFoundError if failure == "missing-node" else AssertionError
