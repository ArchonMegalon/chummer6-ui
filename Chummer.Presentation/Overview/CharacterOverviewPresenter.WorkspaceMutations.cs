...
public sealed partial class CharacterOverviewPresenter
{
    ...
    public async Task<CareerWeaponFireEditorState?> PrepareCareerWeaponFireAsync(
        Guid weaponId,
        CancellationToken ct)
    {
        using PresenterOperationLease operation = EnterPresenterOperation(ct);
        ...
        if (workspace is null || revision <= 0 || weaponId == Guid.Empty)
        {
            Publish(State with { Error = "Open a saved Career runner before firing a Weapon." });
            return null;
        }
        ...
        try
        {
                ...
                || !string.Equals(read.Value.Id.Value, workspace.Value.Value, StringComparison.Ordinal))
            {
                Publish(State with
                {
                    Error = read.Error ?? "The dossier changed before Weapon firing began."
                });
                return null;
            }
            if (read.Value.Document.Format != WorkspaceDocumentFormat.NativeXml)
            {
                Publish(State with { Error = "Weapon firing requires a native XML dossier." });
                return null;
            }

            CareerWeaponFireEditorState editor = CareerWeaponFireEditorProjector.Project(
                read.Value.Document.Content,
                workspace.Value,
                revision,
                weaponId);
        ...
        CancellationToken ct)
    {
            ...
            || State.ContentRevision != request.ExpectedContentRevision)
        {
            Publish(State with
            {
                Error = "This runner changed while Weapon firing was open. Reopen it."
            });
            return;
        }

        await ApplyWorkspaceXmlMutationAsync(
            xml => WorkspaceXmlMutationCatalog.ApplyCareerWeaponFire(xml, request),
            ct).ConfigureAwait(false);
    }
    ...
    public async Task<CareerActiveSkillAdvanceEditorState?> PrepareCareerActiveSkillAdvanceAsync(
        CancellationToken ct)
    {
        using PresenterOperationLease operation = EnterPresenterOperation(ct);
        ct = operation.Token;
        ...
        if (currentWorkspace is null || expectedContentRevision <= 0)
        {
            Publish(State with { Error = "Open a saved career runner before advancing an active skill." });
            return null;
        }
        ...
        try
        {
            ...
            if (!read.Success || read.Value is null)
            {
                Publish(State with { Error = read.Error ?? "Dossier could not be read for active-skill advancement." });
                return null;
            }
            if (!string.Equals(read.Value.Id.Value, currentWorkspace.Value.Value, StringComparison.Ordinal)
                || read.Value.ContentRevision != expectedContentRevision)
            {
                Publish(State with { Error = "The dossier changed before active-skill advancement could begin." });
                return null;
            }
            if (read.Value.Document.Format != WorkspaceDocumentFormat.NativeXml)
            {
                Publish(State with { Error = "Active-skill advancement requires a native XML dossier." });
                return null;
            }
    ...
    public async Task ApplyCareerActiveSkillAdvanceAsync(
        ...
        CancellationToken ct)
    {
            ...
            || requestState.ContentRevision != request.ExpectedContentRevision)
        {
            Publish(State with
            {
                Error = "This runner changed while active-skill advancement was open. Reopen it before saving."
            });
            return;
        }
        ...
        await ApplyWorkspaceXmlMutationAsync(
            request.WorkspaceId,
            request.ExpectedContentRevision,
            xml => WorkspaceXmlMutationCatalog.ApplyCareerActiveSkillAdvance(
                xml,
                request,
                settingsCatalogJson,
                _characterSourceDataResolver),
    ...
    public async Task<CareerKnowledgeSkillAdvanceEditorState?> PrepareCareerKnowledgeSkillAdvanceAsync(
        CancellationToken ct)
    {
        using PresenterOperationLease operation = EnterPresenterOperation(ct);
        ct = operation.Token;
        ...
        if (currentWorkspace is null || expectedContentRevision <= 0)
        {
            Publish(State with
            {
                Error = "Open a saved career runner before advancing a knowledge or language skill."
            });
            return null;
        }
        ...
        try
        {
            ...
            if (!read.Success || read.Value is null)
            {
                Publish(State with
                {
                    Error = read.Error ?? "Dossier could not be read for knowledge-skill advancement."
                });
                return null;
            }
            if (!string.Equals(
                ...
                || read.Value.ContentRevision != expectedContentRevision)
            {
                Publish(State with
                {
                    Error = "The dossier changed before knowledge-skill advancement could begin."
                });
                return null;
            }
            if (read.Value.Document.Format != WorkspaceDocumentFormat.NativeXml)
            {
                Publish(State with
                {
                    Error = "Knowledge-skill advancement requires a native XML dossier."
                });
                return null;
            }
    ...
    public async Task<CharacterCareerKnowledgeSkillAdvanceReceipt?> ApplyCareerKnowledgeSkillAdvanceAsync(
        ...
        CancellationToken ct)
    {
            ...
            || requestState.ContentRevision != request.ExpectedContentRevision)
        {
            Publish(State with
            {
                Error = "This runner changed while knowledge-skill advancement was open. Reopen it before saving."
            });
            return null;
        }

        string? settingsCatalogJson = requestState.Preferences.CharacterSettingsCatalogJson;
        CareerKnowledgeSkillAdvanceMutationResult? prepared = null;
        CharacterCareerKnowledgeSkillAdvanceReceipt? committed = null;
        await ApplyWorkspaceXmlMutationAsync(
            request.WorkspaceId,
            request.ExpectedContentRevision,
            xml =>
            {
                prepared = WorkspaceXmlMutationCatalog.ApplyCareerKnowledgeSkillAdvance(
                    xml,
                    request,
                    settingsCatalogJson,
                    _characterSourceDataResolver);
